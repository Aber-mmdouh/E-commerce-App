...
import { IAuthentication } from '../i-authentication';
...
this.authSubject = new BehaviorSubject<boolean>(false)
  ...
  signup(email:string,password:string){
    ...
    }).pipe(catchError(this.handelerror),tap((resData)=>this.handelAuthentication(resData.email,resData.localId,resData.idToken,+resData.expiresIn)))
  }
    login(email:string,password:string){
    return this.http.post<IAuthentication>('https://identitytoolkit.googleapis.com/v1/accounts:signInWithCustomToken?key=AIzaSyB6xMceEPzN-BwQi0wYEHwI7nVVuWDrwIY',{
      email:email,
      password:password,
      returnSecureToken:true
